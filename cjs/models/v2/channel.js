...
class Channel extends base_1.BaseModel {
    ...
    operations() {
        ...
        ['publish', 'subscribe'].forEach(operationAction => {
            const id = this._json[operationAction] && this._json[operationAction].operationId || `${this.meta().id}_${operationAction}`;
            if (this._json[operationAction]) {
                operations.push(this.createModel(operation_1.Operation, this._json[operationAction], { id, action: operationAction, pointer: `${this._meta.pointer}/${operationAction}` }));
            }
        });
