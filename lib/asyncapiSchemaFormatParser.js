...
function getValidator(version) {
  ...
  if (!validate) {
    const payloadSchema = preparePayloadSchema(asyncapi[String(version)]);
...
function preparePayloadSchema(asyncapiSchema) {
  ...
  //we return $ref and "definitions" and not only "definitions.schema" because some properties of "schema" refer to other definitions, like json schema one
  return {
