...
async function parse({ message, oryginalAsyncAPIDocument, fileFormat, parsedAsyncAPIDocument }) {
  const ajv = new Ajv({
    jsonPointers: true,
    allErrors: true,
    schemaId: 'id',
  ...
  if (!valid) throw new ParserError({
    ...
    title: 'There were errors validating the message payload against the AsyncAPI schema format.',
    parsedJSON: parsedAsyncAPIDocument,
    validationErrors: improveAjvErrors(validate.errors, oryginalAsyncAPIDocument, fileFormat),
  });
}
