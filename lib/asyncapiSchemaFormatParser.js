...
async function parse({ message, originalAsyncAPIDocument, fileFormat, parsedAsyncAPIDocument, path }) {
  const ajv = new Ajv({
    jsonPointers: true,
    allErrors: true,
    schemaId: 'id',
  ...
  if (!valid) throw new ParserError({
    ...
    validationErrors: improveAjvErrors(addFullPathToDataPath(validate.errors, path), originalAsyncAPIDocument, fileFormat),
  });
}
