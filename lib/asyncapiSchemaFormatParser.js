...
const schemas = Object.values(asyncapi).map((schema) => {return Object.values(schema.definitions)}).flat();
const ajv = new Ajv({
  jsonPointers: true,
  allErrors: true,
  schemaId: 'auto',
...
function preparePayloadSchema(asyncapiSchema, version) {
  return {
    $ref: `#/definitions/http://asyncapi.com/definitions/${version}/schema.json`,
