...
function validateServerSecuritySchemaMapping(parsedJSON, asyncapiYAMLorJSON, initialFormat) {
  const srvs = parsedJSON.servers;
  if (!srvs) return true;
  
  const root = 'servers';
  const srvsMap = new Map(Object.entries(srvs));
  const secSchemes =  parsedJSON.components && parsedJSON.components.securitySchemes;
  ...
  //returns Array where first element is the name fo the security schema and the second element is the type of the schema
  const findSecuritySchema = name => {
    const schemaInfo = [];
  ...
  srvsMap.forEach((server, serverName) => {
    ...
    serverSecInfo.forEach(secObj => {
      Object.keys(secObj).forEach(secName => {
        const schema = findSecuritySchema(secName);
        ...
        if (schemaType !== 'oauth2' && schemaType !== 'openIdConnect') {
          const securityObjValue = secObj[String(secName)];
          if (securityObjValue.length) invalidSecurityValues.set(srvrSecurityPath, schemaType);
        }
      });
    });
  });
...
module.exports = {
  ...
  validateOperationId,
  validateServerSecuritySchemaMapping
};