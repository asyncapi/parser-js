...
function validateServerVariables(
  ...
  const notProvidedEnumInExamples = new Map();
  ...
  srvsMap.forEach((val, key) => {
    ...
    if (val.variables) {
      const enumExamples = val.variables;
      const enumExamplesMaps = new Map(Object.entries(enumExamples));
      enumExamplesMaps.forEach((val, key1) => {
        const notProvidedEnumInExamplesinServerVars = notProvidedVariables.get(
          tilde(key)
        );
        if (val.enum && val.examples) {
          const booleanEnumVariablesInExamples = val.enum.includes(
            val.examples
          );
          if (!booleanEnumVariablesInExamples) {
            notProvidedEnumInExamples.set(
              `${tilde(key)}/variables/${tilde(key1)}`,
              notProvidedEnumInExamplesinServerVars
                ? notProvidedEnumInExamplesinServerVars
                : booleanEnumVariablesInExamples
            );
          }
        }
      });
  ...
  if (notProvidedEnumInExamples.size) {
    throw new ParserError({
      type: validationError,
      title:
...
function validateServerSecurity(
  ...
  if (missingSecSchema.size) {
    throw new ParserError({
      ...
      validationErrors: groupValidationErrors(
        root,
        'does not have a corresponding security schema under the components object',
        missingSecSchema,
        asyncapiYAMLorJSON,
        initialFormat
      ),
