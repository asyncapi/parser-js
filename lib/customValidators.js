...
function validateTags(parsedJSON, asyncapiYAMLorJSON, initialFormat) {
  validateRootTags(parsedJSON, asyncapiYAMLorJSON, initialFormat);
  validateOperationTraitTags(parsedJSON, asyncapiYAMLorJSON, initialFormat);
  validateAllChannelsTags(parsedJSON, asyncapiYAMLorJSON, initialFormat);
  validateMessageTraitTags(parsedJSON, asyncapiYAMLorJSON, initialFormat);

  return true;
}

function validateRootTags(parsedJSON, asyncapiYAMLorJSON, initialFormat) {
  if (parsedJSON.tags && hasDuplicateTags(parsedJSON.tags)) {
    throw new ParserError({
      type: validationError,
...
function validateOperationTraitTags(parsedJSON, asyncapiYAMLorJSON, initialFormat) {
  if (parsedJSON?.components?.operationTraits) {
    Object.keys(parsedJSON.components.operationTraits).forEach(operationTrait => {
      // eslint-disable-next-line security/detect-object-injection
      if (hasDuplicateTags(parsedJSON.components.operationTraits[operationTrait].tags)) {
        throw new ParserError({
          type: validationError,
          title: `Invalid operationTrait \"${operationTrait}\", there can't be any duplicate tags in the operationTrait object`,
          parsedJSON,
          validationErrors: groupValidationErrors(
            'operationTrait',
            'operationTrait contains duplicate tags',
            new Map(),
            asyncapiYAMLorJSON,
            initialFormat
          ),
...
function validateAllChannelsTags(parsedJSON, asyncapiYAMLorJSON, initialFormat) {
  const chnls = parsedJSON.channels;
  if (!chnls) return true;

  const chnlsMap = new Map(Object.entries(chnls));
  chnlsMap.forEach(channel => validateChannelTags(parsedJSON, channel, asyncapiYAMLorJSON, initialFormat));
}

function validateChannelTags(parsedJSON, channel, asyncapiYAMLorJSON, initialFormat) {
  Object.keys(channel).forEach(channelOperation => {
    // eslint-disable-next-line security/detect-object-injection
    const operation = channel[channelOperation];

    if (operation?.tags && hasDuplicateTags(operation.tags)) {
      throw new ParserError({
        type: validationError,
        title: `Invalid operation in channel \"${channelOperation}\", there can't be any duplicate tags`,
        parsedJSON,
        validationErrors: groupValidationErrors(
          'channels',
    ...
    if (operation?.message?.tags && hasDuplicateTags(operation.message.tags)) {
      throw new ParserError({
        type: validationError,
        title: `Invalid message in channel \"${channelOperation}\", there can't be any duplicate tags`,
        parsedJSON,
        validationErrors: groupValidationErrors(
          'channels',
          'channels contains duplicate tags',
...
function validateMessageTraitTags(parsedJSON, asyncapiYAMLorJSON, initialFormat) {
  if (parsedJSON?.components?.messageTraits) {
    Object.keys(parsedJSON.components.messageTraits).forEach(messageTrait => {
      // eslint-disable-next-line security/detect-object-injection
      if (hasDuplicateTags(parsedJSON.components.messageTraits[messageTrait].tags)) {
        throw new ParserError({
          type: validationError,
          title: `Invalid messageTrait \"${messageTrait}\", there can't be any duplicate tags in the messageTrait object`,
          parsedJSON,
          validationErrors: groupValidationErrors(
            'messageTrait',
            'messageTrait contains duplicate tags',
            new Map(),
            asyncapiYAMLorJSON,
            initialFormat
          ),
...
function hasDuplicateTags(tags) {
  const tagNames = tags.map(item => item.name);
  return tagNames.some((item, idx) => tagNames.indexOf(item) !== idx);
}
