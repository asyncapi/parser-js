...
function validateTags(parsedJSON, asyncapiYAMLorJSON, initialFormat) {
  ...
  const invalidMessageTraits = validateMessageTraitTags(parsedJSON);
...
function validateRootTags(parsedJSON) {
  ...
  if (duplicateNames && duplicateNames.length) {
    invalidRoot.set('', duplicateNames.toString());
  }
  ...
  if (parsedJSON && parsedJSON.components && parsedJSON.components.operationTraits) {
    Object.keys(parsedJSON.components.operationTraits).forEach(operationTrait => {
      ...
      if (duplicateNames.length) {
        ...
        invalidOperationTraits.set(operationTraitsPath, duplicateNames.toString());
      }
    });
  }
...
function validateChannelTags(invalidChannels, channel, channelName) {
  Object.keys(channel).forEach(channelOperation => {
    ...
    const operationPath = `${tilde(channelName)}/${channelOperation}`;

    let duplicateNames = operation && operation.tags && getDuplicateTagNames(operation.tags);
    if (duplicateNames && duplicateNames.length) {
      invalidChannels.set(operationPath, duplicateNames.toString());
    }

    duplicateNames = operation && operation.message && operation.message.tags && getDuplicateTagNames(operation.message.tags);
    if (duplicateNames && duplicateNames.length) {
      invalidChannels.set(`${operationPath}/message`, duplicateNames.toString());
    }
  });
}

function validateMessageTraitTags(parsedJSON) {
...
function getDuplicateTagNames(tags) {
  ...
  const duplicateNames = new Set(tagNames);
