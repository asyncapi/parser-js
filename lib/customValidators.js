  ...
  srvsMap.forEach((server, serverName) => {
    ...
    serverSecInfo.forEach(secObj => {
      Object.keys(secObj).forEach(secName => {
        ...
        const schema = findSecuritySchema(secName, secSchemesMap);
 ...
 * @param  {Map} secSchemesMap map of the security schema object
 ...
 */
function findSecuritySchema(securityName, secSchemesMap) {
