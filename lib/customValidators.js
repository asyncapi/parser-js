...
const validTypes = {
  'bindings/amqp/bindingVersion': {type: 'string'},
  'bindings/amqp/is': {type: 'string',rangeOfValues: ['queue', 'routingKey']},
  'bindings/amqp/exchange': {type: 'object'},
  'bindings/amqp/exchange/autoDelete': {type: 'boolean'},
  'bindings/amqp/exchange/durable': {type: 'boolean'},
  'bindings/amqp/exchange/name': {type: 'string', maxLength: 255},
  'bindings/amqp/exchange/vhost': {type: 'string'},
  'bindings/amqp/exchange/type': {type: 'string'},
  'bindings/amqp/queue': {type: 'object'},
  'bindings/amqp/queue/autoDelete': {type: 'boolean'},
  'bindings/amqp/queue/durable': {type: 'boolean'},
  'bindings/amqp/queue/name': {type: 'string'},
  'bindings/amqp/queue/vhost': {type: 'string'},
};
...
function validateChannels(parsedJSON, asyncapiYAMLorJSON, initialFormat) {
  ...
  const invalidTypes = new Map(); //return object for invalid types in channel.
  const invalidValue = new Map(); //return object for invalid values in channel.
  const invalidLength = new Map(); //return object for invalid length in channel.
  ...
  chnlsMap.forEach((val, key) => {
    ...
    for (const [validKey,value] of Object.entries(validTypes)) {
      const result = navigateObject(val,validKey);
      if (result !== undefined) {
        if (typeof result !== value.type) {
          invalidTypes.set(`${tilde(key)}/${validKey}`, typeof result);
        } else if (value.rangeOfValues && value.rangeOfValues.indexOf(result) === -1) {
          invalidValue.set(`${tilde(key)}/${validKey}`, result);
        } else if (value.maxLength && value.maxLength < result.length) {
          invalidLength.set(`${tilde(key)}/${validKey}`, value.maxLength);
        }
      }
    }
  ...
  const lengthValidationErrors = groupValidationErrors(
    'channels',
  ...
  const valueValidationErrors = groupValidationErrors(
    'channels',
  ...
  const typeValidationErrors = groupValidationErrors(
    'channels',
    'has invalid type of',
    invalidTypes,
    asyncapiYAMLorJSON,
  ...
  const allValidationErrors = parameterValidationErrors.concat(
    nameValidationErrors, typeValidationErrors, valueValidationErrors, lengthValidationErrors
  );

  //channel variable validation: throw exception if channel validation failes
  if (notProvidedParams.size || invalidChannelName.size || invalidTypes.size || valueValidationErrors.size || lengthValidationErrors.size) {
