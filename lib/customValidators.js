...
function validateServerVariables(parsedJSON, asyncapiYAMLorJSON, initialFormat) {
  ...
  srvsMap.forEach((val, key) => {
    ...
    const notProvidedServerVars = notProvidedVariables.get(key.replace(regex, '~1'));
    if (!variables) return;

    const missingServerVariables = getMissingProps(variables, val.variables);
    if (!missingServerVariables.length) return;

    notProvidedVariables.set(key.replace(regex, '~1'), 
...
function validateChannelParams(parsedJSON, asyncapiYAMLorJSON, initialFormat) {
  ...
  chnlsMap.forEach((val, key) => {
    ...
    const notProvidedChannelParams = notProvidedParams.get(key.replace(regex, '~1'));
    if (!variables) return;
    ...
    notProvidedParams.set(key.replace(regex, '~1'), 
...
function validateOperationId(parsedJSON, asyncapiYAMLorJSON, initialFormat) {
  const chnls = parsedJSON.channels;
  if (!chnls) return true;
  const chnlsMap = new Map(Object.entries(chnls));
  ...
  const addDuplicateToMap = (op, k, opName) => {
    const operationId = op.operationId;
    const operationPath = `${ k.replace(regex, '~1') }/${ opName }/operationId`;
  ...
  chnlsMap.forEach((val,key) => {
    ...
    if (sub) addDuplicateToMap(sub, key, 'subscribe');
    if (pub) addDuplicateToMap(pub, key, 'publish');
  });
