 ...
 * @property {string} patternProperties Crawl all schemas in patternProperties
 * @property {string} definitions Crawl all schemas in definitions
 * @property {string} dependencies Crawl all schemas in dependencies
 */
...
function traverseSchema(schema, callback, prop, schemaTypesToIterate) {
  ...
  if (schema.patternProperties() !== undefined && schemaTypesToIterate.includes(SchemaTypesToIterate.patternProperties)) {
    const props = schema.patternProperties();
    for (const [prop, patternPropertiesSchema] of Object.entries(props)) {
      const circularProps = schema.circularProps();
      if (circularProps !== undefined && circularProps.includes(prop)) continue;
      traverseSchema(patternPropertiesSchema, callback, prop, schemaTypesToIterate);
    }
  }
  if (schema.definitions() !== undefined && schemaTypesToIterate.includes(SchemaTypesToIterate.definitions)) {
    const props = schema.definitions();
    for (const [prop, definitionsSchema] of Object.entries(props)) {
      console.log(JSON.stringify(props));
      const circularProps = schema.circularProps();
      if (circularProps !== undefined && circularProps.includes(prop)) continue;
      traverseSchema(definitionsSchema, callback, prop, schemaTypesToIterate);
    }
  }
  if (schema.dependencies() !== undefined && schemaTypesToIterate.includes(SchemaTypesToIterate.dependencies)) {
    const props = schema.dependencies();
    if (props !== null)
      for (const [prop, dependenciesSchema] of Object.entries(props)) {
        const circularProps = schema.circularProps();
        if (circularProps !== undefined && circularProps.includes(prop)) continue;
        traverseSchema(dependenciesSchema, callback, prop, schemaTypesToIterate);
      }
  }
...
function traverseAsyncApiDocument(doc, callback, schemaTypesToIterate) {
  ...
  if (doc.hasComponents() && schemaTypesToIterate.includes(SchemaTypesToIterate.components)) {
    ...
    if (schemaTypesToIterate.includes(SchemaTypesToIterate.parameters)) {
      Object.values(doc.components().parameters()).forEach(p => {
        traverseSchema(p.schema(), callback, null, schemaTypesToIterate);
      });
    }
    if (schemaTypesToIterate.includes(SchemaTypesToIterate.headers)) {
      Object.values(doc.components().messageTraits()).forEach(m => {
        traverseSchema(m.headers(), callback, null, schemaTypesToIterate);
      });
    }
  }
}
