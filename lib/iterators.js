...
function traverseSchema(schema, callback, prop, schemaTypesToIterate) {
  ...
  const checkSchemas = (props) => {
    for (const [prop_, combineSchema] of Object.entries(props)) {
      const circularProps = schema.circularProps();
      if (circularProps !== undefined && circularProps.includes(prop_)) continue;
      traverseSchema(combineSchema, callback, prop_, schemaTypesToIterate);
    }
  };
  if (schema.patternProperties() !== undefined && schemaTypesToIterate.includes(SchemaTypesToIterate.patternProperties)) {
    checkSchemas(schema.patternProperties());
  }
  if (schema.definitions() !== undefined && schemaTypesToIterate.includes(SchemaTypesToIterate.definitions)) {
    checkSchemas(schema.definitions());
  }
  if (schema.dependencies() !== undefined && schemaTypesToIterate.includes(SchemaTypesToIterate.dependencies)) {
    ...
    if (props !== null)
      for (const [prop_, dependenciesSchema] of Object.entries(props)) {
        const circularProps = schema.circularProps();
        if (circularProps !== undefined && circularProps.includes(prop_)) continue;
        traverseSchema(dependenciesSchema, callback, prop_, schemaTypesToIterate);
      }
  }
