...
const regexToken = new RE2(/^Unexpected token.*position\s+(\d+)/i);
const regexEndof = new RE2(/^Unexpected end of JSON.*/i);
...
module.exports = (txt, reviver, context = 20) => {
  try {
    ...
    const syntaxErr = e.message.match(regexToken);
    const errIdxBrokenJson = e.message.match(regexEndof) ? txt.length - 1 : null;
