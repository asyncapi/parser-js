  ...
  allSchemas() {
    ...
    if (this.hasChannels()) {
      this.channelNames().forEach(channelName => {
        ...
        Object.values(channel.parameters()).forEach(p => {
          if (p.schema()) {
            schemas = new Map([...schemas , ...recursiveSchemaFinder(p.schema())]);
          }
        });
        ...
        if (channel.hasPublish()) {
          channel.publish().messages().forEach(m => {
            if (m.headers()) {
              schemas = new Map([...schemas , ...recursiveSchemaFinder(m.headers())]);
            }

            if (m.payload()) {
              schemas = new Map([...schemas , ...recursiveSchemaFinder(m.payload())]);
            }
          });
        }
        if (channel.hasSubscribe()) {
    ...
    if (this.hasComponents()) {
      Object.values(this.components().schemas()).forEach(s => {
        schemas = new Map([...schemas , ...recursiveSchemaFinder(s)]);
      });
    }
...
function recursiveSchemaFinder(schema) {
  var schemas = new Map();
  let findMoreSchemasIn = (schemaToSearch) => {
    const recursiveSchemas = recursiveSchemaFinder(schemaToSearch);
    schemas = new Map([...schemas, ...recursiveSchemas]);
  }
    ...
    switch (schema.type()) {
      case 'object':
        ...
        for (const [_, propertySchema] of Object.entries(props)) {
          findMoreSchemasIn(propertySchema);
        }
        break;
      case 'array':
        if (Array.isArray(schema.items())) {
          schema.items().forEach(arraySchema => {
            findMoreSchemasIn(arraySchema);
          });
        } else {
          findMoreSchemasIn(schema.items());
        }
        break;
    }
  }else{
    //Since it is not a type, check for allOf, oneOf, anyOf
    let checkCombiningSchemas = (combineArray) => {
      if(combineArray !== null && combineArray.length > 0){
        combineArray.forEach(combineSchema => {
          schemas = new Map([...schemas, ...recursiveSchemaFinder(combineSchema)]);;
        });
      }
    }
