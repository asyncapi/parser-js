...
class AsyncAPIDocument extends Base {
   ...
   */
  traverseSchemas(callback){
    traverseAsyncApiDocument(this, callback);
  }
}


function markRecursive(doc){
  var seenObj = []
  var lastSchema =  []
  ...
  const callback = (schema, propName, type) => {
    switch(type){
      case SCHEMA_CALLBACK_TYPE.END_SCHEMA:
        lastSchema.pop();
        seenObj.pop()
        break;
      case SCHEMA_CALLBACK_TYPE.NEW_SCHEMA:
...
function recursiveSchema(schemaContent, callback) {
  ...
  return crawl(schemaContent, callback);
}
const SCHEMA_CALLBACK_TYPE = {
 ...
 */
function crawl(schema, callback, prop) {
  if (schema.isCircular() || ! callback(schema, prop, SCHEMA_CALLBACK_TYPE.NEW_SCHEMA)) return;

  if (schema.type() !== undefined) {
    switch (schema.type()) {
    case 'object':
      recursiveSchemaObject(schema, callback);
      break;
    case 'array':
  ...
  callback(schema, prop, SCHEMA_CALLBACK_TYPE.END_SCHEMA)
}
 ...
 * @param {FoundSchemaCallback} callback
 */
function traverseAsyncApiDocument(doc, callback) {
  if (doc.hasChannels()) {
    doc.channelNames().forEach(channelName => {
      const channel = doc.channel(channelName);
 ...
 *         schema {Schema}: the found schema.
 */
function recursiveSchemaObject(schema, callback) {
  if (schema.additionalProperties() !== undefined && typeof schema.additionalProperties() !== 'boolean') {
    const additionalSchema = schema.additionalProperties();
    crawl(additionalSchema, callback)
  }
  if (schema.properties() !== null) {
    ...
    for (const [prop, propertySchema] of Object.entries(props)) {
      ...
      crawl(propertySchema, callback, prop);
    }
  }
}
...
function recursiveSchemaArray(schema, callback) {
  if (schema.additionalItems() !== undefined) {
    ...
    crawl(additionalArrayItems, callback);
  }

  if (schema.items() !== null) {
    if (Array.isArray(schema.items())) {
      schema.items().forEach(arraySchema => {
        crawl(arraySchema, callback);
      });
    } else {
      crawl(schema.items(), callback);
    }
  }
}
