...
class AsyncAPIDocument extends Base {
  constructor(...args) {
    ...
    assignUidToPropertySchemas(this);
...
/**
 * Assign object property keys as uid for the property schema.
 * 
 * @param {AsyncAPIDocument} doc 
 */
function assignUidToPropertySchemas(doc) {
  let callback = (schema) => {
    if (schema.type() != null && schema.type() == "object") {
      let props = schema.properties();
      for (const [propertyKey, propertySchema] of Object.entries(props)) {
        propertySchema.json()['x-parser-schema-id'] = propertyKey;
      }
    }
  };
...
/**
 * Recursively go through each schema and call callback.
 * 
