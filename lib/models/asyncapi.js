...
class AsyncAPIDocument extends Base {
  ...
  /**
   * @returns {string[]} an array of server names
   */
  serverNames() {
   ...
   * @returns {Server} server for provided name
   */
  server(name) {
  ...
  /**
   * @returns {string|null} default content type if exists, otherwise null
   */
  defaultContentType() {
   ...
   * @returns {Channel} channel for the provided name
   */
  channel(name) {
  ...
  /**
   * @returns {boolean} true if it has components, otherwise null
   */
  hasComponents() {
   ...
   * 
   * @param {TraverseSchemas} callback callback
   * @param {SchemaTypesToIterate[]} schemaTypesToIterate schema types to iterate
   */
 ...
 * 
 * @private
 * @param {AsyncAPIDocument} doc asyncAPI document
 */
