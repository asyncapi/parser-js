...
class AsyncAPIDocument extends Base {
  ...
  static stringify(doc, space) {
    if (doc.hasCircular()) {
      return JSON.stringify(doc.json(), refReplacer(), space);
    }
    return JSON.stringify(doc.json(), undefined, space);
  }

  /* eslint-disable sonarjs/cognitive-complexity */
  /**
   * Converts a valid stringified AsyncAPIDocument instance into an object.
   * 
   * @param {string} doc A valid stringified AsyncAPIDocument instance.
   * @returns {AsyncAPIDocument}
   */
  static parse(doc) { // NOSONAR
    const objToPath = new Map();
    const pathToObj = new Map();
    const parsedJSON = JSON.parse(doc);

    // if is not circular
    if (!parsedJSON[String(xParserSpecParsed)]) {
    ...
    function traverse(parent, field) {
      ...
      if (objOrPath === '$ref:$' || ref === '$ref:$') {
        parent[String(field)] = parsedJSON;
      }
  ...
  /* eslint-enabled sonarjs/cognitive-complexity */
}
