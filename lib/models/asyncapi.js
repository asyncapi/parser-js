...
function assignIdToAnonymousSchemas(doc) {
  ...
  let assignIdRecursively = (schema) => {
    if (schema == null) {
      return;
    }
    if (!schema.$id()) {
      ...
      switch (schema.type()) {
        case 'object':
          ...
          for (const [_, propertySchema] of Object.entries(props)) {
            assignIdRecursively(propertySchema);
          }
          break;
        case 'array':
          if (Array.isArray(schema.items())) {
            schema.items().forEach(arraySchema => {
              assignIdRecursively(arraySchema);
            });
          } else {
            assignIdRecursively(schema.items());
          }
          break;
      }
    } else {
      ...
      let checkCombiningSchemas = (combineArray) => {
        if (combineArray !== null && combineArray.length > 0) {
          combineArray.forEach(combineSchema => {
            assignIdRecursively(combineSchema);;
          });
        }
      }
  ...
  if (doc.hasChannels()) {
    doc.channelNames().forEach(channelName => {
      ...
      Object.values(channel.parameters()).forEach(p => {
        assignIdRecursively(p.schema());
      });

      if (channel.hasPublish()) {
        channel.publish().messages().forEach(m => {
          assignIdRecursively(m.headers());
          assignIdRecursively(m.payload());
        });
      }
      if (channel.hasSubscribe()) {
        channel.subscribe().messages().forEach(m => {
