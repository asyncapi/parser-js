...
class AsyncAPIDocument extends Base {
   ...
   * 
   * @param {TraverseSchemas} cb 
   * @param {SchemaTypesToIterate[]} schemaTypesToIterate
   */
  traverseSchemas(cb, schemaTypesToIterate) {
    traverseAsyncApiDocument(this, cb, schemaTypesToIterate);
  }
}
...
function markCircularSchemas(doc) {
  ...
  //cb to use for iterating through the schemas
 ...
 * @param {SchemaTypesToIterate[]} schemaTypesToIterate
 */
function traverseSchema(schema, cb, prop, schemaTypesToIterate) {
  ...
  if (cb(schema, prop, SchemaIteratorCallbackType.NEW_SCHEMA) === false) return;

  if (schema.type() !== undefined) {
    switch (schema.type()) {
    case 'object':
      recursiveSchemaObject(schema, cb, schemaTypesToIterate);
      break;
    case 'array':
      recursiveSchemaArray(schema, cb, schemaTypesToIterate);
      break;
    }
  } else {
    traverseCombinedSchemas(schema, cb, schemaTypesToIterate);
  }
  cb(schema, prop, SchemaIteratorCallbackType.END_SCHEMA);
}
 ...
 * @param {SchemaTypesToIterate[]} schemaTypesToIterate
 */
function traverseCombinedSchemas(schema, cb, schemaTypesToIterate) {
  ...
  const checkCombiningSchemas = (combineArray) => {
    if (combineArray !== null && combineArray.length > 0) {
      combineArray.forEach(combineSchema => {
        traverseSchema(combineSchema, cb, null, schemaTypesToIterate);
      });
    }
  };
...
/**
 * Go through each channel and for each parameter, and message payload and headers recursively call the cb for each schema.
 * 
 * @private
 * @param {AsyncAPIDocument} doc 
 * @param {FoundSchemaCallback} cb
 * @param {SchemaTypesToIterate[]} schemaTypesToIterate
 */
function traverseAsyncApiDocument(doc, cb, schemaTypesToIterate) {
  if (!schemaTypesToIterate) {
    schemaTypesToIterate = [
      SchemaTypesToIterate.allOfs,
      SchemaTypesToIterate.anyOfs,
      SchemaTypesToIterate.arrays,
      SchemaTypesToIterate.components,
      SchemaTypesToIterate.headers,
      SchemaTypesToIterate.objects,
      SchemaTypesToIterate.oneOfs,
      SchemaTypesToIterate.parameters,
      SchemaTypesToIterate.payloads
    ];
  }
  if (doc.hasChannels()) {
    doc.channelNames().forEach(channelName => {
      const channel = doc.channel(channelName);
      traverseChannel(channel, cb, schemaTypesToIterate);
    });
  }
  if (doc.hasComponents() && schemaTypesToIterate.includes(SchemaTypesToIterate.components)) {
    Object.values(doc.components().schemas()).forEach(s => {
      traverseSchema(s, cb, null, schemaTypesToIterate);
    });
  }
}
 ...
 * @param {SchemaTypesToIterate[]} schemaTypesToIterate
 */
function traverseChannel(channel, cb, schemaTypesToIterate) {
  if (schemaTypesToIterate.includes(SchemaTypesToIterate.parameters)) {
    Object.values(channel.parameters()).forEach(p => {
      traverseSchema(p.schema(), cb, null, schemaTypesToIterate);
    });
  }
  if (channel.hasPublish()) {
    channel.publish().messages().forEach(m => {
      traverseMessage(m, cb, schemaTypesToIterate);
    });
  }
  if (channel.hasSubscribe()) {
    channel.subscribe().messages().forEach(m => {
 ...
 * @param {SchemaTypesToIterate[]} schemaTypesToIterate
 */
function traverseMessage(message, cb, schemaTypesToIterate) {
  if (schemaTypesToIterate.includes(SchemaTypesToIterate.headers)) {
    traverseSchema(message.headers(), cb, null, schemaTypesToIterate);
  }
  if (schemaTypesToIterate.includes(SchemaTypesToIterate.payloads)) {
    traverseSchema(message.payload(), cb, null, schemaTypesToIterate);
  }
}
...
function assignIdToAnonymousSchemas(doc) {
  ...
  traverseAsyncApiDocument(doc, cb);
}

/**
 * Recursively go through schema of object type and execute cb.
 * 
 * @private
 ...
 * @param {SchemaTypesToIterate[]} schemaTypesToIterate
 */
function recursiveSchemaObject(schema, cb, schemaTypesToIterate) {
  if (schema.additionalProperties() !== undefined && typeof schema.additionalProperties() !== 'boolean') {
    const additionalSchema = schema.additionalProperties();
    traverseSchema(additionalSchema, cb, null, schemaTypesToIterate);
  }
  if (schema.properties() !== null) {
    ...
    for (const [prop, propertySchema] of Object.entries(props)) {
      ...
      traverseSchema(propertySchema, cb, prop, schemaTypesToIterate);
    }
  }
}

/**
 * Recursively go through schema of array type and execute cb.
 * 
 * @private
 ...
 * @param {SchemaTypesToIterate[]} schemaTypesToIterate
 */
function recursiveSchemaArray(schema, cb, schemaTypesToIterate) {
  if (schema.additionalItems() !== undefined) {
    const additionalArrayItems = schema.additionalItems();
    traverseSchema(additionalArrayItems, cb, null, schemaTypesToIterate);
  }

  if (schema.items() !== null) {
    if (Array.isArray(schema.items())) {
      schema.items().forEach(arraySchema => {
        traverseSchema(arraySchema, cb, null, schemaTypesToIterate);
      });
    } else {
      traverseSchema(schema.items(), cb, null, schemaTypesToIterate);
    }
  }
}
