    ...
    switch(type){
      ...
      case SchemaIteratorCallbackType.NEW_SCHEMA:
        ...
        if(seenObj.includes(schemaJson)){
          var schemaToUse = lastSchema.length > 0 ? lastSchema[lastSchema.length-1] : schema
...
function traverseAsyncApiDocument(doc, cb, schemaTypesToIterate) {
  if(!schemaTypesToIterate){
    ...
    for (const key in SchemaTypesToIterate) {
      schemaTypesToIterate.push(SchemaTypesToIterate[key])
    }
  }
  if (doc.hasChannels()) {
    doc.channelNames().forEach(channelName => {
