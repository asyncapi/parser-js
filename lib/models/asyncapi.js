...
function assignNameToAnonymousMessages(doc) {
   ...
   */
  const addNameToKey = (messages, number) => {
    messages.forEach(m => {
      if (m.name() === undefined) {
...
function assignIdToAnonymousSchemas(doc) {
  //Traverse all messages and cumulate the name
  const messages = doc.allMessages();
  messages.entries(([messageUid, message]) => {
    let cumulateSchemaName = messageUid;
    const callback = (schema, propName, callbackType) => {
      if (callbackType === SchemaIteratorCallbackType.END_SCHEMA && 
        propName !== undefined) {
        cumulateSchemaName = cumulateSchemaName.replace(propName, '');
      }
      if (propName !== undefined) {
        cumulateSchemaName += propName;
      }
      if (!schema.uid()) {
        schema.json()[String(xParserSchemaId)] = cumulateSchemaName;
      }
    };
    traverseMessage(message, callback);
  });
