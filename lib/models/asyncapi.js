...
class AsyncAPIDocument extends Base {
  ...
  allMessages() {
    ...
    if (this.hasComponents()) {
      for(const [key, value] of Object.entries(this.components().messages())){
        messages.set(value.uid(), value);
      }
    }
  ...
  allSchemas() {
    ...
    if (this.hasComponents()) {
      for(const [key, value] of Object.entries(this.components().schemas())){
...
function assignCommpentMessagesId(doc){
