...
class AsyncAPIDocument extends Base {
  ...
  static parse(doc) {
    ...
    traverseStringifiedDoc(parsedJSON, undefined, objToPath, pathToObj);
 ...
 * @param {Map} pathToObj
 */
function traverseStringifiedDoc(parent, field, objToPath, pathToObj) {
  ...
  if (objOrPath === Object(objOrPath)) {
    for (const f in objOrPath) {
      traverseStringifiedDoc(objOrPath, f, objToPath, pathToObj);
    }
  }
}
