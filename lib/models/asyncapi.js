  ...
  allChannels() {
    ...
    if (this.hasChannels()) {
      this.channelNames().forEach(channelName => {
        ...
        channelHashes.add(JSON.stringify((channel._json)));
      });
    }

    if (this.hasComponents()) {
      Object.entries(this.components().channels()).forEach(([key, value]) => {
        const hash = JSON.stringify((value._json));

        // if channel already exists, we skip it.
        if (channelHashes.has(hash)) {
          return;
        }
        
        // In the case there is a name collision, we use the base64 encoded hash of the channel object as name.
        if (channels.has(key)) {
  ...
  allServers() {
    ...
    if (this.hasServers()) {
      this.serverNames().forEach(serverName => {
        ...
        serverHashes.add(JSON.stringify((server._json)));
      });
    }
    ...
    if (this.hasComponents()) {
      Object.entries(this.components().servers()).forEach(([key, value]) => {
        ...
        // if server already exists, we skip it.
        if (serverHashes.has(hash)) {
          return;
        }

        // In the case there is a name collision, we use the base64 encoded hash of the server object as name.
        if (servers.has(key)) {
          key = Buffer.from(hash).toString('base64');
        }
