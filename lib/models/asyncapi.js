...
function isCircular(obj, seenObjects) {
  const json = obj.json();
  if (obj && typeof obj === 'object') {
    if (!seenObjects.includes(json)) return false;
    json[String(xParserCircle)] = true;
    return true;
  }
}
...
function crawl(schema, seenObj, callback) {
  if (isCircular(schema, seenObj)) return;
    ...
    const checkCombiningSchemas = (combineArray) => {
      if (combineArray !== null && combineArray.length > 0) {
        combineArray.forEach(combineSchema => {
          crawl(combineSchema, seenObj, callback); ;
        });
      }
    };
...
function recursiveSchemaObject(schema, seenObj, callback) {
  if (schema.additionalProperties() !== undefined && typeof schema.additionalProperties() !== 'boolean') {
    ...
    crawl(additionalSchema, seenObj, callback);
  }
  if (schema.properties() !== null) {
    const props = schema.properties();
    for (const [, propertySchema] of Object.entries(props)) {
      crawl(propertySchema, seenObj, callback);
    }
  }
}
...
function recursiveSchemaArray(schema, seenObj, callback) {
  if (schema.additionalItems() !== undefined) {
    ...
    crawl(additionalArrayItems, seenObj, callback);
  }

  if (schema.items() !== null) {
    if (Array.isArray(schema.items())) {
      schema.items().forEach(arraySchema => {
        crawl(arraySchema, seenObj, callback);
      });
    } else {
      crawl(schema.items(), seenObj, callback);
    }
  }
}
