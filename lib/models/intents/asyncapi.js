...
const isVersion2 = (document) => {
  return document.asyncapi === '2.0.0' ||
    document.asyncapi === '2.1.0' ||
    document.asyncapi === '2.2.0' ||
    document.asyncapi === '2.3.0';
};
...
class IntentAsyncAPIDocument {
  ...
  hasContentType(contentType) {
    return this.defaultContentType() === contentType;
  }
  ...
  applicationPublishOperations() { 
    ...
    return relatedChannels.map((channel) => {
      //TODO what is considered operation type?
      return new IntentOperation(this.document, channel, 'publish');
    });
  }
  ...
  applicationSubscribeOperations() {
    ...
    return relatedChannels.map((channel) => {
      //TODO what is considered operation type?
      return new IntentOperation(this.document, channel, 'subscribe');
    });
  }
  ...
  operations() {
    if (isVersion2(this.document)) {
      return this.channels().map((channel) => {
        const operations = [];
        //TODO what is considered operation type?
        if (channel.subscribe !== undefined) operations.push('subscribe');
        if (channel.publish !== undefined) operations.push('publish');
