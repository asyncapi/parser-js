...
const { toJS, findRefs, getLocationOf, improveAjvErrors, getCurlyBracesEl, groupExistingElements } = require('./utils');
...
const PARSERS = {};
const ERROR_TYPE_VALIDATION = 'validation-errors';
...
async function parse(asyncapiYAMLorJSON, options = {}) {
  ...
  try {
    ...
    if (parsedJSON.servers) validateServerVariables(parsedJSON);
    if (parsedJSON.channels) validateChannelParams(parsedJSON);
...
function validateChannelParams (parsedJSON) {
  ...
  if (!providedParams.length) {
    throw new ParserError({
      type: ERROR_TYPE_VALIDATION,
      title: `Channel key has parameters specified that are not described with parameter object: ${notProvidedParams}`,
      parsedJSON,
    });
  }
}
