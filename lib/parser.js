...
const validationError = 'schema-parser-not-registered';
...
async function customDocumentOperations(parsedJSON, asyncapiYAMLorJSON, initialFormat, options, schemaValidationErrors) {
  ...
  const customComponentsError = await customComponentsMsgOperations(parsedJSON, asyncapiYAMLorJSON, initialFormat, options, schemaValidationErrors);
  const customChannelsError = await customChannelsOperations(parsedJSON, asyncapiYAMLorJSON, initialFormat, options, schemaValidationErrors);
  ...
  if(schemaValidationErrors.length) {
    throw new ParserError({
      ...
      parsedJSON,
      validationErrors: [...customChannelsError,...customComponentsError],
    });
  }
}

async function validateAndConvertMessage(msg, originalAsyncAPIDocument, fileFormat, parsedAsyncAPIDocument, pathToPayload, options, schemaValidationErrors) {
  ...
  const customSchemaParser = PARSERS[String(schemaFormat)];
  
  if (!customSchemaParser) {
    const errorMap = new Map();
    if(options.component){
      errorMap.set(`messages/${options.opname}`, schemaFormat);  
    }
    else{
      errorMap.set(`${tilde(options.channelName)}/${options.opname}/message/${schemaFormat}`, schemaFormat);
    }
    schemaValidationErrors.push(errorMap);
  }

  else{
    await customSchemaParser({
      schemaFormat,
      message: msg,
      defaultSchemaFormat,
  ...
  Object.entries(parsedJSON.channels).forEach(([channelName, channel]) => {
    promisesArray.push(...OPERATIONS.map(async (opName) => {
      ...
      for (const m of messages) {
        await validateAndConvertMessage(m, asyncapiYAMLorJSON, initialFormat, parsedJSON, pathToPayload,{channelName: channelName,opName: opName}, schemaValidationErrors);
      }
      if (schemaValidationErrors.length) {
...
async function customComponentsMsgOperations(parsedJSON, asyncapiYAMLorJSON, initialFormat, options, schemaValidationErrors) {
  ...
  Object.entries(parsedJSON.components.messages).forEach(([messageName, message]) => {
    ...
    promisesArray.push(validateAndConvertMessage(message, asyncapiYAMLorJSON, initialFormat, parsedJSON, pathToPayload, { component: true, messageName: messageName }, schemaValidationErrors));
  });
  if (schemaValidationErrors.length) {
    const errors = schemaValidationErrors;
    schemaValidationErrors.length = 0;
