...
const xParserOriginalSchemaFormat = 'x-parser-original-schema-format';
...
async function customDocumentOperations(parsedJSON, asyncapiYAMLorJSON, initialFormat, options) {
  ...
  await customMessagesOperations(parsedJSON, asyncapiYAMLorJSON, initialFormat, options);
...
async function validateAndConvertMessage(msg, originalAsyncAPIDocument, fileFormat, parsedAsyncAPIDocument, pathToPayload) {
  //indicator that the message has already been validated before
  if (xParserOriginalSchemaFormat in msg) return;
  ...
  msg[xParserOriginalSchemaFormat] = schemaFormat;
}
...
/**
 * Triggers additional operations on the AsyncAPI components/messages like traits application, validation and conversion
 *
 * @private
 * 
 * @param  {Object} parsedJSON parsed AsyncAPI document
 ...
 * @param  {Object} options Configuration options.
 */
async function customMessagesOperations(parsedJSON, asyncapiYAMLorJSON, initialFormat, options) {
