...
async function validateAndConvertMessage(msg, originalAsyncAPIDocument, fileFormat, parsedAsyncAPIDocument, pathToPayload) {
  ...
  //checkpoint
  const messagesWithUnknownSchema = new Map();

  //const schemaFormat

  messagesWithUnknownSchema.set(pathToPayload,[msg.schemaFormat]);
  ...
  if (!parser) {
    throw new ParserError({
      ...
      validationErrors: groupValidationErrors(
        ...
        messagesWithUnknownSchema,
        originalAsyncAPIDocument,
        fileFormat
      ),
      title: 'The file does not correspond to a supported schema format',
      detail: `The following message payload "${pathToPayload}" has "${schemaFormat}" schema format which isn't supported by parser instance. Add a missed format parser through the "registerSchemaParser" function.`
    });
  }
  await parser({
    schemaFormat,
