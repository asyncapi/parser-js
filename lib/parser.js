...
async function validateAndConvertMessage(msg, channelName ,originalAsyncAPIDocument, fileFormat, parsedAsyncAPIDocument, pathToPayload) {
  ...
  errorMap.set(`${tilde(channelName)}/${Operation.name}/${msg}/${schemaFormat}`, schemaFormat);
  if (!parser) {
  ...
  Object.entries(parsedJSON.channels).forEach(([channelName, channel]) => {
    promisesArray.push(...OPERATIONS.map(async (opName) => {
      ...
      for (const m of messages) {
        await validateAndConvertMessage(m,channelName ,asyncapiYAMLorJSON, initialFormat, parsedJSON, pathToPayload);
      }
...
async function customComponentsMsgOperations(parsedJSON, asyncapiYAMLorJSON, initialFormat, options) {
  ...
  Object.entries(parsedJSON.components.messages).forEach(([messageName, message]) => {
    ...
    promisesArray.push(validateAndConvertMessage(message,messageName,asyncapiYAMLorJSON, initialFormat, parsedJSON, pathToPayload));
  });
  if (schemaValidationErrors.length) {
    throw new ParserError({
      type: validationError,
