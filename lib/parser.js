  ...
  for (const channelName in js.channels) {
    ...
    const convert = OPERATIONS.map(async (opName) => {
      ...
      if (op) {
        ...
        const path = `/channels/${ channelName }/${ opName }/message/payload`; 
        if (msg) await validateAndConvertMessage(msg, asyncapiYAMLorJSON, initialFormat, js, path);
      }
    });
...
async function validateAndConvertMessage(msg, originalAsyncAPIDocument, fileFormat, parsedAsyncAPIDocument, path) {
