...
async function validateAndConvertMessage(msg,name ,originalAsyncAPIDocument, fileFormat, parsedAsyncAPIDocument, pathToPayload,options) {
  ...
  Object.entries(parsedJSON.channels).forEach(([channelName, channel]) => {
    promisesArray.push(...OPERATIONS.map(async (opName) => {
      ...
      for (const m of messages) {
        await validateAndConvertMessage(m, opName, asyncapiYAMLorJSON, initialFormat, parsedJSON, pathToPayload,{channelName: channelName});
      }
...
async function customComponentsMsgOperations(parsedJSON, asyncapiYAMLorJSON, initialFormat, options) {
  ...
  Object.entries(parsedJSON.components.messages).forEach(([messageName, message]) => {
    ...
    promisesArray.push(validateAndConvertMessage(message,messageName,asyncapiYAMLorJSON, initialFormat, parsedJSON, pathToPayload, { component: true }));
  });
  if (schemaValidationErrors.length) {
    const errors = schemaValidationErrors;
    schemaValidationErrors.length = 0;
