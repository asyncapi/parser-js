...
const {toJS,findRefs,getLocationOf,improveAjvErrors,getDefaultSchemaFormat,groupValidationErrors} = require('./utils');
  ...
  try {
    ...
    if (
      ...
      throw new ParserError({
        ...
        parsedJSON,
        validationErrors: [
          getLocationOf('/asyncapi', asyncapiYAMLorJSON, initialFormat),
        ],
      });
    }
...
async function handleCircularRefs(
  ...
  await dereference(refParser, parsedJSON, initialFormat, asyncapiYAMLorJSON, {
    ...options,
...
async function validateAndConvertMessage(
  ...
  if (xParserMessageParsed in msg && msg[String(xParserMessageParsed)] === true)
    return;
  const defaultSchemaFormat = getDefaultSchemaFormat(
  ...
  if (!parser) {
    throw new ParserError({
      ...
      validationErrors: groupValidationErrors(
        'servers',
        'server does not have a corresponding schema object',
        new Map(),
        fileFormat,
        parser
      ),
...
async function customComponentsMsgOperations(
  ...
  Object.entries(parsedJSON.components.messages).forEach(
    ([messageName, message]) => {
      if (options.applyTraits) {
        applyTraits(message);
      }
