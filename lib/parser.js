...
async function parse(asyncapiYAMLorJSON, options = {}) {
  ...
  try {
    ...
    await customDocumentOperations(parsedJSON, options);
  } catch (e) {
    if (e instanceof ParserError) throw e;
    throw new ParserError({
      type: 'unexpected-error',
...
async function customDocumentOperations(js, options) {
  if (js.servers) validateServerVariables(js);
  if (!js.channels) return;
  
  validateChannelParams(js);
