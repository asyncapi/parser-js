  ...
  for (const channelName in js.channels) {
    ...
    const convert = OPERATIONS.map(async (opName) => {
      ...
      if (op) {
        ...
        if (msg) await validateAndConvertMessage(msg, asyncapiYAMLorJSON, initialFormat, js);
      }
    });
...
async function validateAndConvertMessage(msg, oryginalAsyncAPIDocument, fileFormat, parsedAsyncAPIDocument) {
