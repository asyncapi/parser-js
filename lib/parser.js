...
const { toJS, findRefs, getLocationOf, improveAjvErrors, hasRef } = require('./utils');
...
async function parse(asyncapiYAMLorJSON, options = {}) {
  ...
  //parser is not able to handle fully resolved circular references so we first need to ignore them and leave circular $refs to successfully validate the document
  //this id done pair to advice from Ajv creator https://github.com/ajv-validator/ajv/issues/1122#issuecomment-559378449
  ...
  try {
    ...
    parsedJSON = await dereference(parsedJSON, initialFormat, asyncapiYAMLorJSON, options);
    ...
    await handleCircularRefs(parsedJSON, initialFormat, asyncapiYAMLorJSON, options);
...
async function handleCircularRefs(parsedJSON, initialFormat, asyncapiYAMLorJSON, options) {
  if (!hasRef(parsedJSON)) return;

  options.dereference = {circular: true};
  parsedJSON = await dereference(parsedJSON, initialFormat, asyncapiYAMLorJSON, options);
  parsedJSON = sc(parsedJSON);
}
