...
async function parse(asyncapiYAMLorJSON, options = {}) {
  const schemaValidationErrors = new Map();
  let parsedJSON;
  let initialFormat;
...
async function customDocumentOperations(parsedJSON, asyncapiYAMLorJSON, initialFormat, options, schemaValidationErrors) {
  ...
  if (schemaValidationErrors.size) {
    throw new ParserError({
      ...
      validationErrors: groupValidationErrors(
        'channels',
        'uses unsupported schema format',
...
async function validateAndConvertMessage(msg, originalAsyncAPIDocument, fileFormat, parsedAsyncAPIDocument, pathToPayload, options, schemaValidationErrors) {
  ...
  if (!customSchemaParser) {
    if (options.component) {
      schemaValidationErrors.set(`messages/${options.messageName}`, schemaFormat);  
    } else {
      schemaValidationErrors.set(`${tilde(options.channelName)}/${options.opName}/message/${schemaFormat}`, schemaFormat);
    }
  } else {
  ...
  Object.entries(parsedJSON.channels).forEach(([channelName, channel]) => {
    promisesArray.push(...OPERATIONS.map(async (opName) => {
      ...
      for (const m of messages) {
        await validateAndConvertMessage(m, asyncapiYAMLorJSON, initialFormat, parsedJSON, pathToPayload,{channelName,opName}, schemaValidationErrors);
      }
    }));
  });
...
async function customComponentsMsgOperations(parsedJSON, asyncapiYAMLorJSON, initialFormat, options, schemaValidationErrors) {
  ...
  Object.entries(parsedJSON.components.messages).forEach(([messageName, message]) => {
    ...
    promisesArray.push(validateAndConvertMessage(message, asyncapiYAMLorJSON, initialFormat, parsedJSON, pathToPayload, { component: true, messageName }, schemaValidationErrors));
  });
  await Promise.all(promisesArray);
}
