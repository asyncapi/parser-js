...
const version2Validators = require('./version2/customValidators.js');
const { toJS, findRefs, getLocationOf, improveAjvErrors, getDefaultSchemaFormat, isVersion2} = require('./utils');
const AsyncAPIVersion2Document = require('./version2/models/asyncapi');
const IntentAsyncAPIDocument = require('./models/asyncapi').IntentAsyncAPIDocument;
 ...
 */
async function parseToJson(asyncapiYAMLorJSON, options) {
  let parsedJSON;
  let initialFormat;
 ...
 * @returns {Promise<IntentAsyncAPIDocument>} The parsed AsyncAPI document wrapped in intents.
 */
async function parse(asyncapiYAMLorJSON, options = {}) {
  const json = await parseToJson(asyncapiYAMLorJSON, options);
  let document;
  if (isVersion2(json)) {
    document = new AsyncAPIVersion2Document(json);
  }
  return new IntentAsyncAPIDocument(document);
}
...
function parseFromUrl(url, fetchOptions, options) {
  //Why not just adding a default to the arguments list?
...
async function customDocumentOperations(parsedJSON, asyncapiYAMLorJSON, initialFormat, options) {
  if (isVersion2(parsedJSON)) {
    version2Validators.validateServerVariables(parsedJSON, asyncapiYAMLorJSON, initialFormat);
    version2Validators.validateServerSecurity(parsedJSON, asyncapiYAMLorJSON, initialFormat, SPECIAL_SECURITY_TYPES);
    if (!parsedJSON.channels) return;
  
    version2Validators.validateTags(parsedJSON, asyncapiYAMLorJSON, initialFormat);
    version2Validators.validateChannels(parsedJSON, asyncapiYAMLorJSON, initialFormat);
    version2Validators.validateOperationId(parsedJSON, asyncapiYAMLorJSON, initialFormat, OPERATIONS);
  }
