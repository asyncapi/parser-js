...
async function parse(asyncapiYAMLorJSON, options = {}) {
  ...
  try {
    ...
    if (!js.asyncapi || !asyncapi[js.asyncapi]) {
      throw new ParserError({
        type: 'missing-or-unsupported-version',
        title: `AsyncAPI version is missing or unsupported: ${js.asyncapi}.`,
        parsedJSON: js,
      });
    }
  ...
  try {
    ...
    if (!valid) throw new ParserError({
      ...
      parsedJSON: js,
      validationErrors: betterAjvErrors(asyncapi[js.asyncapi], js, validate.errors, { format: 'js' }),
    });
