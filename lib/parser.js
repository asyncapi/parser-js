...
function parseFromUrl(url, fetchOptions, options) {
  ...
  //To not breat the API by changing argument position and to silet the linter it is just better to move adding
...
async function customDocumentOperations(parsedJSON, asyncapiYAMLorJSON, initialFormat, options) {
  if (parsedJSON.servers) validateServerVariables(parsedJSON, asyncapiYAMLorJSON, initialFormat);
