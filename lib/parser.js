...
const { toJS, findRefs, getLocationOf, improveAjvErrors, getDefaultSchemaFormat, groupValidationErrors, tilde } = require('./utils');
...
async function parse(asyncapiYAMLorJSON, options = {}) {
  ...
  try {
    ...
    await customDocumentOperations(parsedJSON, asyncapiYAMLorJSON, initialFormat, options, schemaValidationErrors);
...
async function customDocumentOperations(parsedJSON, asyncapiYAMLorJSON, initialFormat, options, schemaValidationErrors) {
  ...
  await customComponentsMsgOperations(parsedJSON, asyncapiYAMLorJSON, initialFormat, options, schemaValidationErrors);
  await customChannelsOperations(parsedJSON, asyncapiYAMLorJSON, initialFormat, options, schemaValidationErrors);
}

async function validateAndConvertMessage(msg,name ,originalAsyncAPIDocument, fileFormat, parsedAsyncAPIDocument, pathToPayload, options, schemaValidationErrors) {
  ...
  const parser = PARSERS[String(schemaFormat)];
  const errorMap = new Map();
  if(options.component){
    errorMap.set(`messages/${name}/${schemaFormat}`, schemaFormat);  
  }
  else{
    errorMap.set(`${tilde(options.channelName)}/${name}/${msg}/${schemaFormat}`, schemaFormat);
  }
  
  if (!parser) {
    schemaValidationErrors.push(new ParserError({
      type: 'schema-parser-not-registered',
      title: 'The specification has an unsupported schema format',
      detail:
        'You shall not pass, no schema parser available that can parse this schema. The good news is that we have one for Avro, just install @asyncapi/avro-schema-parser and use registerSchemaParser to register it with the current parser',
      validationErrors: groupValidationErrors(
        'channels',
        'channel does not have a corresponding schema object',
        errorMap,
 ...
 */
async function customChannelsOperations(parsedJSON, asyncapiYAMLorJSON, initialFormat, options, schemaValidationErrors) {
  const promisesArray = [];
  Object.entries(parsedJSON.channels).forEach(([channelName, channel]) => {
    promisesArray.push(...OPERATIONS.map(async (opName) => {
      ...
      for (const m of messages) {
        await validateAndConvertMessage(m, opName, asyncapiYAMLorJSON, initialFormat, parsedJSON, pathToPayload,{channelName: channelName}, schemaValidationErrors);
      }
 ...
 * @param  {ParserOptions} options Configuration options. {@link ParserOptions}
 */
async function customComponentsMsgOperations(parsedJSON, asyncapiYAMLorJSON, initialFormat, options, schemaValidationErrors) {
  ...
  Object.entries(parsedJSON.components.messages).forEach(([messageName, message]) => {
    ...
    promisesArray.push(validateAndConvertMessage(message,messageName,asyncapiYAMLorJSON, initialFormat, parsedJSON, pathToPayload, { component: true }, schemaValidationErrors));
  });
  if (schemaValidationErrors.length) {
    const errors = schemaValidationErrors;
    schemaValidationErrors.length = 0;
