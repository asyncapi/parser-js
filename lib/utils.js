 ...
 * @returns {object} the model extended with mixins
 */
utils.mix = (model, ...mixins) => {
  let duplicatedMethods = false;
  /**
   * @param {Array<object>} mixin array with mixins to extend the model with
   * @returns {boolean | object} true if mixin is equals to object, otherwise duplicated methods
   */
 ...
 * @returns {Array} with parsed values
 */
 ...
 * @param {string} str url
 * @returns {Array} with url parameters
 */
 ...
 * @param {Array} arr with properties
 * @param {object} obj with properties
 * @returns {Array} with non existing properties in provided object
 */
