...
/**
 * Returns an array with all strings in between curly braces, including braces
 */
utils.getCurlyBracesEl = str => {
...
/**
 * Returns an array with values that map to the keys of provided object
 */
utils.groupExistingElements = (arr, obj) => {
  const groupedElements = {existEl: [], noEl: []};
  arr = arr.map(val => val.replace(/[{}]/g, ''));

  if (!obj) {
    groupedElements.noEl = arr;
    return groupedElements;
  }
  
  groupedElements.existEl = arr.filter(val => {
    const exists = obj.hasOwnProperty(val);

    if (!exists) groupedElements.noEl = groupedElements.noEl.concat(val);
    return exists;
  });
...
/**
 * Returns array of errors for channel params and server variables validation
 */
utils.groupValidationErrors = (root, errorMessage, errorElements, asyncapiYAMLorJSON, initialFormat) => {
  const error = [];
  const regex = new RE2(/\//g);

  Object.keys(errorElements).forEach((el) => {
    error.push({
      title: `${el} ${errorMessage}: ${errorElements[el]}`,
      location: utils.getLocationOf(root + el.replace(regex, '~1'), asyncapiYAMLorJSON, initialFormat)
    });
  });
